...
> 状态：架构方向已确认，本文作为实施与评审基线。  
> 工作分支：`feat/mrv2-hybrid-pcp`（基线 `414d80ec0` / `li1how/pcp-integration` tip）；按 §6 从 M0 重开，不整包复用旧 Hybrid PCP 实现。  
...
- Ascend `maybe_build_ascend_pcp_manager` 已改走 `validate_ascend_pcp_config` / Hybrid 专用门禁，**不再调用**上游 MLA-only `PCPManager.validate_config`；未支持组合由 capability / 配置 fail-fast
...
| R8 | **Capability 类型归属** | 权威定义在 `worker/v2/pcp/capability.py`；`attention/.../hybrid_pcp` 仅 re-export，避免 Manager→attention 反向依赖 | 已落地 |
| R9 | **GQA cache-first 硬约束** | Hybrid GQA prefill 强制 `PrefillCacheHit`；`forward_impl` 传入 placeholder KV，真实 KV 只来自刚写入的本地 cache | 已落地 |
