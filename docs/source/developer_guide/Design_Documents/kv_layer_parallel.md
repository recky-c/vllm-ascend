...
The planner stores the placement result in `KVCacheConfig`:
...
`kvpp_rank` records the rank used for physical allocation. The model runner
validates it against `get_kvpp_group().rank_in_group`; this protects layouts
where PCP or elastic rank construction changes the communication ordering.
...
1. vLLM owns configuration, the KVPP process group, physical cache planning,
   scratch aliases, and the logical layer-to-owner map.
...
3. `KVPPScheduler` owns active-page selection, batch/forward lifecycle,
   one-layer-ahead prefetch, scratch lifetime, and cross-rank notifications.
4. `MemFabricMTEKVPPTransport` owns only registration, staging, MTE copies,
   unpack, and transport completion.
...
The scheduler receives its transport by dependency injection; the common
transport module contains protocols and metadata only, with no backend factory
or environment-based dispatch. MemFabric MTE is the only data plane. Owners
...
`vllm/v1/core/kv_cache_placement.py`. The existing cache planner only invokes
that policy at its physical-projection boundary; transport and runtime
lifecycle code never enter vLLM core.
...
- Placement policy: `vllm/v1/core/kv_cache_placement.py`
