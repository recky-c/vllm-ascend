...
- Keep vLLM changes limited to generic cache-placement hooks and keep KVPP
  policy and transport in vLLM Ascend.
...
The compatibility CLI option is forwarded to
`additional_config.kvpp_size` and parsed by Ascend's `KVPPConfig`. The TP size
...
Ownership is deterministic. The engine-side placement hook and each worker
recompute it from the same model configuration and logical cache names, so no
KVPP-only fields are added to `KVCacheConfig` or serialized to workers.
...
1. vLLM exposes generic physical-placement and finalization hooks during KV
   cache planning, plus a generic post-binding callback for device runners.
2. vLLM Ascend owns `KVPPConfig`, the KVPP process group, owner calculation,
   physical cache planning, and scratch aliases.
3. The Ascend model runner is the composition root. It discovers executable
   attention layers, creates the MTE transport, injects it into
   `KVPPScheduler`, and binds the scheduler through a generic attention hook.
4. `KVPPScheduler` owns active-page selection, forward lifecycle,
   one-layer-ahead prefetch, scratch lifetime, and cross-rank notifications.
5. `MemFabricMTEKVPPTransport` owns registration, cache metadata, staging,
   MTE copies, unpack, and transport completion.

Attention implementations depend only on `LayerwiseKVCacheHook` with three
operations: `enter_layer`, `wait_for_layer`, and `leave_layer`. They do not
...
`vllm_ascend/v1/core/kv_cache_placement.py`. The vLLM planner invokes the
platform only at its physical-projection and finalization boundaries;
transport and runtime lifecycle code never enter vLLM core.

Before vLLM starts, source the MemFabric Hybrid environment and configure its
store:
...
- Cache planner integration: `vllm/v1/core/kv_cache_utils.py`
...
- Generic attention hook: `vllm_ascend/attention/layerwise_cache.py`
