...
from vllm_ascend.worker.v2.model_runner import flashcomm_dispatch_wrapper
from vllm_ascend.worker.v2.sp_utils import (
    all_gather_hidden_states,
    flashcomm_enabled,
)
...
def test_all_gather_hidden_states_trims_flashcomm_padding():
    ...
    with patch(
        ...
        result = all_gather_hidden_states(
            local_hidden_states,
            num_tokens=5,
        )
...
def test_flashcomm_dense_threshold_and_moe_behavior():
    ...
    with (
        ...
        assert not flashcomm_enabled(config, 1000)
        assert flashcomm_enabled(config, 1001)
    ...
    with (
        ...
        assert flashcomm_enabled(config, 1)
