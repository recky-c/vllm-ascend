...
def test_prepare_attn_prepares_kvpp_only_for_full_graph_capture():
    state = AscendModelState.__new__(AscendModelState)
    state.max_model_len = 1024
    state.kvpp_capture_stage = MagicMock()
    input_batch = MagicMock()
    input_batch.num_reqs = 1
    input_batch.num_reqs_after_padding = 1
    input_batch.num_tokens = 1
    input_batch.num_tokens_after_padding = 1
    input_batch.num_scheduled_tokens = np.array([1], dtype=np.int32)
    input_batch.query_start_loc_np = np.array([0, 1], dtype=np.int32)
    input_batch.query_start_loc = torch.tensor([0, 1], dtype=torch.int32)
    input_batch.seq_lens = torch.tensor([1], dtype=torch.int32)
    input_batch.dcp_local_seq_lens = None
    input_batch.seq_lens_np = np.array([1], dtype=np.int32)
    input_batch.positions = torch.arange(1, dtype=torch.int64)
    input_batch.attn_state = None
    block_tables = (torch.zeros((1, 1), dtype=torch.int32),)
    ...
    state.kvpp_capture_stage.assert_called_once_with(
        block_tables, input_batch
    )
