...
def _kv_cache_config(
    ...
    *,
    ...
    size = num_blocks * block_stride if block_stride else num_blocks * spec.page_size_bytes
...
def test_get_kv_cache_spec_aligns_attention_and_orders_mamba_last(
    ...
    with patch(
        "vllm_ascend.worker.v2.attn_utils.Attention",
        FakeAttention,
    ):
...
def test_hybrid_model_state_rejects_duplicate_or_stale_step_ids():
    state = object.__new__(AscendMambaHybridModelState)
    first_step = SimpleNamespace(step_id=1)
    second_step = SimpleNamespace(step_id=2)

    state.bind_hybrid_prepared_step(first_step)
    with pytest.raises(RuntimeError, match="already bound"):
        state.bind_hybrid_prepared_step(second_step)

    state._hybrid_prepared_step = None
    with pytest.raises(RuntimeError, match="newer"):
        state.bind_hybrid_prepared_step(first_step)

    state.bind_hybrid_prepared_step(second_step)
    assert state._hybrid_prepared_step is second_step
