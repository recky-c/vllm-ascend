...
def allocate_kvpp_cache(
    ...
    layouts = {
        name: build_kvpp_layer_layout(bundle, plan.tensor_specs, kv_cache_config.num_blocks)
    ...
    scratch = [torch.zeros(scratch_size, dtype=torch.int8, device=device) for _ in range(2)] if scratch_size else []
    caches = {}
    target_index = 0
    for name, (layout, size) in layouts.items():
        ...
        if owner is None or owner == plan.kvpp_rank:
            ...
            buffer = scratch[target_index % 2]
        for cache_name, parts in layout.items():
            caches[cache_name] = tuple(buffer.narrow(0, offset, size) for offset, size in parts)
