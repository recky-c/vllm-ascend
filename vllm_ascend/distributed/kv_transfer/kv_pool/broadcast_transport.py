...
class BroadcastKVPPTransport:
    ...
    def __init__(
        ...
        layer_buffers: dict[str, tuple[torch.Tensor, ...]],
    ) -> None:
    ...
    def prefetch(self, layer_name: str, cache_ready: Any, transfer_stream: Any) -> None:
        with torch.npu.stream(transfer_stream):
            ...
            works = []
            for buffer in self._layer_buffers[layer_name]:
                work = dist.broadcast(
                    buffer, src=self._owner_global_ranks[layer_name], group=self._device_group, async_op=True
                )
                work.wait()
                works.append(work)
