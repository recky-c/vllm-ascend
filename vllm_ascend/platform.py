...
class NPUPlatform(Platform):
    ...
    def set_additional_forward_context(
        ...
        # MoE AllGather pads local batches to this width before PCP AG.
        # Runner passes num_tokens_after_padding (already equal across PCP ranks
        # after partition). Missing this field makes _EXTRA_CTX return None.
        pcp_size = vllm_config.parallel_config.prefill_context_parallel_size
        max_tokens_across_pcp = 0
        if pcp_size > 1 and num_tokens is not None:
            max_tokens_across_pcp = int(num_tokens)
