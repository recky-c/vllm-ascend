...
#
# Algorithm (same math as V1, rewritten structure):
...
#   6. All ranks take the last valid rank's correct as decode final_state
#
...
def _recompute_local_h(
    *,
    ...
    initial (local s0). Rank 0's token outputs are already correct under
    causal order; decode end-state is taken from the chained final, not
    from this recompute.
...
def correct_pcp_prefill_ssm_state(
    *,
    ...
    Returns:
        ...
        share the same corrected end ``final_state``.
    """
    ...
    # Rank 0: local token outputs already use the true sequence start.
    # Decode end-state is ``final_state`` from the chain above (not local final).
    # Ranks with no local prefill tokens: outputs are unused; keep AscendC h/v_new.
